...
export function getHealthVariant(health: boolean): Variant {
  return health ? 'success' : 'error'
}

export function getAdviceActionVariant(action: AdviceAction): Variant {
  switch (action) {
    case AdviceAction.BUY:
      return 'success'
    case AdviceAction.SELL:
      return 'error'
...
export function getOrderSideVariant(side: OrderSide): Variant {
  switch (side) {
    case OrderSide.BUY:
      return 'success'
    case OrderSide.SELL:
...
export function getPositionStatusVariant(status: PositionStatus): Variant {
  switch (status) {
...
export function getPnLVariant(value: number): Variant {
  if (value >= 0) {
    return 'success'
  } else {
    return 'error'
